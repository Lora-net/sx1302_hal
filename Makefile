...
.PHONY: all clean install install_conf libtools libloragw packet_forwarder util_net_downlink util_chip_id util_test_usb

all: libtools libloragw packet_forwarder util_net_downlink util_chip_id util_test_usb
...
install:
	...
	$(MAKE) install -e -C util_test_usb
