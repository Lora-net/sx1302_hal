...
int sx1261_com_w(lgw_com_type_t com_type, void *com_target, sx1261_op_code_t op_code, uint8_t *data, uint16_t size) {
    ...
    switch (com_type) {
        case LGW_COM_SPI:
            com_stat = sx1261_spi_w(com_target, op_code, data, size);
            break;
        case LGW_COM_USB:
            com_stat = sx1261_usb_w(com_target, op_code, data, size);
            break;
        default:
...
int sx1261_com_r(lgw_com_type_t com_type, void *com_target, sx1261_op_code_t op_code, uint8_t *data, uint16_t size) {
    ...
    switch (com_type) {
        case LGW_COM_SPI:
            com_stat = sx1261_spi_r(com_target, op_code, data, size);
            break;
        case LGW_COM_USB:
            com_stat = sx1261_usb_r(com_target, op_code, data, size);
            break;
        default:
            printf("ERROR: wrong communication type (SHOULD NOT HAPPEN)\n");
            com_stat = LGW_COM_ERROR;
