...
#define CONTEXT_TIMESTAMP       lgw_context.ftime_cfg
...
static lgw_context_t lgw_context = {
    ...
    .ftime_cfg = {
        .ftime_enable = false,
        .ftime_mode = LGW_FTIME_MODE_ALL_SF
    },
    .debug_cfg = {
        .nb_ref_payload = 0,
...
int lgw_ftime_setconf(struct lgw_conf_ftime_s * conf) {
    ...
    CONTEXT_TIMESTAMP.ftime_enable = conf->ftime_enable;
    CONTEXT_TIMESTAMP.ftime_mode = conf->ftime_mode;
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    if ((nb_pkt_found > 0) && (CONTEXT_TIMESTAMP.ftime_enable == true)) {
...
uint32_t lgw_time_on_air(struct lgw_pkt_tx_s *packet) {
    ...
    if (packet->modulation == MOD_LORA) {
        ...
        printf("INFO: LoRa packet ToA: %u ms\n", toa_ms);
