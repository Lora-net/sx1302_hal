...
#define CAL_ITER                1 /* Number of calibration iterations */
...
#include "src/text_cal_sx1257_24_Oct_3.var"
...
int cal_tx_dc_offset(uint8_t rf_chain, uint32_t freq_hz, uint8_t dac_gain, uint8_t mix_gain, uint8_t radio_type, int32_t f_offset, int32_t i_offset, int32_t q_offset, bool full_log, bool use_agc) {
    int i;
    uint16_t reg;
...
//    DEBUG_PRINTF("rf_chain:%u, freq_hz:%u, dac_gain:%u, mix_gain:%u, radio_type:%d\n", rf_chain, freq_hz, dac_gain, mix_gain, radio_type);
    ...
    if (use_agc == true) {
        ...
        sx1302_agc_mailbox_write(2, 0); /* amp */
        sx1302_agc_mailbox_write(1, 0); /* phi */
    ...
    if (full_log == true) {
        printf("i_offset:%d q_offset:%d f_offset:%d dac_gain:%d mix_gain:%d dec_gain:%d => ", i_offset, q_offset, f_offset, dac_gain, mix_gain, dec_gain);
    } else {
...
int test_freq_scan(uint8_t rf_chain, bool full_log) {
    ...
    for (f = 0; f < 256; f++)
    {
        cal_tx_dc_offset(rf_chain, rf_rx_freq[rf_chain], dac_gain[0], mix_gain[0], rf_radio_type[rf_chain], f, 0, 0, full_log, false);
...
int test_iq_offset(uint8_t rf_chain, uint8_t f_offset, bool full_log, bool use_agc) {
    ...
    for (j = 0; j < nb_gains; j++) {
        for (l = 10; l < 18; l+=1)
        {
            for (m = 0; m < 10; m+=1)
            {
                cal_tx_dc_offset(rf_chain, rf_rx_freq[rf_chain], dac_gain[j], mix_gain[j], rf_radio_type[rf_chain], f_offset, l, m, full_log, use_agc);
                if ((quit_sig == 1) || (exit_sig == 1)) {
                    return 0;
                }
            }
